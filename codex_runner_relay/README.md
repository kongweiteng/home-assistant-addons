...
`0.2.19` 在 `0.2.18` 数据面与 ACK 语义完全不变的前提下，将安装 renderer 的精确滚动兼容扩展到 Runner `0.3.20`。Relay 继续原样转发 `expected_control_revision`，不解析两个 revision 域、不拥有 Desktop 状态，也不放宽任何失败关闭边界；`desktop_revision_conflict` 仍会关闭连接。
...
安装 renderer 精确接受 Runner `0.3.20`，并保留已发布 `0.3.6`、`0.3.11`、`0.3.12`、`0.3.13`、`0.3.14`、`0.3.15`、`0.3.16`、`0.3.17`、`0.3.18` 与 `0.3.19` 的滚动兼容；除此之外的版本仍 fail closed。Relay 到 Controller 的 IPv4、Registry/lease 所有权、ticket/enrollment 不落盘、单 Runner 连接绑定和其他错误失败关闭边界保持不变。
