...
`0.2.35` 的安装目录新增精确 Runner `0.3.35`，并保留 `0.3.34` 与全部原有回滚版本。WSS envelope、ACK、连接绑定和权限边界不变；旧五字段计划模式目录由 Controller 验签和完整校验后正常消费，Relay 不解析或删除 Host、快照、Bridge 或任务内容。源码发布不代表 HAOS Relay 或 Mac Runner 已完成升级验收。
...
- 返回脚本下载固定 installer，核对字节大小和 SHA-256，再下载并核对固定平台 bundle。Relay `0.2.35` 的滚动目录精确接受 Python `3.11.13`、Codex `0.146.0` 与 Runner `0.3.35`，同时保留安装目录中已冻结的全部旧版本 bootstrap；其他 Runner 版本仍拒绝。
