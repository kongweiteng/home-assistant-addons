...
`0.2.21` 在 `0.2.20` 数据面与 ACK 语义完全不变的前提下，将安装 renderer 的精确滚动兼容扩展到 Runner `0.3.23`，并保留已发布 `0.3.22`。Relay 继续原样转发 Desktop host 事件、送达收据、排队消息命令和 revision 字段，不解析其业务内容、不拥有 Desktop 状态，也不放宽任何失败关闭边界；`desktop_revision_conflict` 仍会关闭连接。
