...
def create_server(
    ...
    *,
    ...
    class Handler(BaseHTTPRequestHandler):
        server_version = "WeixinGateway/0.4.0"
...
function renderPollerControls(status){const enabled=Boolean(status.poller_enabled),override=status.poller_override||'跟随配置默认';q('pollerDesired').textContent=enabled?'已开启':'已关闭';q('pollerStart').disabled=enabled;q('pollerStop').disabled=!enabled;q('pollerControlText').textContent=`当前运行态：${status.poller_state} · desired：${enabled?'enabled':'disabled'} · 覆盖：${override} · 配置默认：${status.poller_default_enabled?'enabled':'disabled'} · revision：${status.poller_revision}`}
