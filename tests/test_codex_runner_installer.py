...
class RunnerInstallerCatalogTests(unittest.TestCase):
    ...
    def test_valid_pinned_manifest_builds_single_line_linux_and_macos_commands(self) -> None:
        ...
        self.assertEqual(linux["runner_version"], "0.3.38")
    ...
    def test_pinned_manifest_body_avoids_runtime_network_dependency(self) -> None:
        ...
        self.assertEqual(status["runner_version"], "0.3.38")
    ...
    def test_packaged_runner_0336_manifest_matches_frozen_release_digest(self) -> None:
        package_root = Path(codex_controller.__file__).parent
        body = (package_root / "runner_manifest_v0336.json").read_bytes()
        self.assertEqual(
            hashlib.sha256(body).hexdigest(),
            "c3f4851a5a74d5468070d7b963b40ebde9215a812a7be5b6b9293d59102c3890",
        )
        catalog = RunnerInstallerCatalog(
            "https://github.com/kongweiteng/home-assistant-addons/releases/download/codex-runner-v0.3.36/manifest.json",
            hashlib.sha256(body).hexdigest(),
            "wss://runner.example.com/v1/connect",
            pinned_manifest_body=body,
            opener=ForbiddenOpener(),
        ...
        self.assertIn('with_name("runner_manifest_v0338.json")', main_source)

    def test_packaged_runner_0337_manifest_matches_frozen_release_digest(self) -> None:
        package_root = Path(codex_controller.__file__).parent
        body = (package_root / "runner_manifest_v0337.json").read_bytes()
        self.assertEqual(
            hashlib.sha256(body).hexdigest(),
            "bee36625eb710510df66027d474b8d3d9bb0864a07c1b04af9c0cdc3e0daccce",
        )
        catalog = RunnerInstallerCatalog(
            "https://github.com/kongweiteng/home-assistant-addons/releases/download/codex-runner-v0.3.37/manifest.json",
            hashlib.sha256(body).hexdigest(),
            "wss://runner.example.com/v1/connect",
            pinned_manifest_body=body,
            opener=ForbiddenOpener(),
    ...
    def test_packaged_runner_0338_manifest_matches_frozen_release_digest(self) -> None:
        package_root = Path(codex_controller.__file__).parent
        body = (package_root / "runner_manifest_v0338.json").read_bytes()
        self.assertEqual(
            hashlib.sha256(body).hexdigest(),
            "72930fbccb7f6be03f2603899a00afc812f09d5adb4679e603715b6c8afa2534",
        )
        catalog = RunnerInstallerCatalog(
            "https://github.com/kongweiteng/home-assistant-addons/releases/download/codex-runner-v0.3.38/manifest.json",
            hashlib.sha256(body).hexdigest(),
            "wss://runner.example.com/v1/connect",
            pinned_manifest_body=body,
            opener=ForbiddenOpener(),
