...
HA Operations Broker 是 Home Assistant 的独立操作授权与最小执行边界。`0.5.1` 在 restart-only 基础上提供恢复/备份证据凭据隔离、不可变策略/适配器/基线/备份证据绑定和 SQLite 持久 singleton/resource lease，并兼容 HAOS 省略 `installed` 字段的 Add-on 信息响应。唯一受支持的写动作仍是重启精确白名单中的 Add-on。
