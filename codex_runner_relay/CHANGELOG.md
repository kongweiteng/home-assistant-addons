...
- 安装目录新增 Runner `0.3.38` 精确兼容，并完整保留 `0.3.37` 与全部既有回滚版本。
- WSS envelope、原始摘要 ACK、连接身份与失败关闭边界不变；Bridge 被动恢复和桌面不跳转边界全部由 Runner 负责，Relay 不解析或拥有这些状态。

## 0.2.37 — released

- 安装目录新增 Runner `0.3.37` 精确兼容，并完整保留 `0.3.36` 与全部既有回滚版本。
- WSS envelope、原始摘要 ACK、连接身份与失败关闭边界不变；任务目录分页和 App-owned 管理后台增强均由 Runner 负责，Relay 不解析或拥有这些状态。

## 0.2.36 — released

- 安装目录精确接受 Runner `0.3.36`，并完整保留 `0.3.35` 与全部既有回滚版本。
- WSS envelope、原始摘要 ACK、连接身份与失败关闭边界不变；Codex App runtime 兼容、Bridge 稳定性与库存同步逻辑均由 Runner 负责，Relay 不解析或拥有这些状态。

## 0.2.35 — released

- 安装目录精确接受 Runner `0.3.35`，并完整保留 `0.3.34` 与全部既有回滚版本。
- WSS envelope、原始摘要 ACK、连接身份与失败关闭边界不变；旧计划模式目录的验签、规范化和完整内容校验只发生在 Controller。

## 0.2.34 — released

- 安装目录精确接受 Runner `0.3.34`，并完整保留 `0.3.33` 与全部既有回滚版本。
- WSS、ACK、连接身份和失败关闭边界不变；旧 Host/snapshot 队头压缩仅由 Runner 对已有唯一更高版本替代的 durable 状态投影执行。
...
## 0.2.28 — local candidate

- 安装目录精确接受 Runner `0.3.28`，并完整保留 `0.3.27` 与全部原有回滚版本。
- WSS envelope、存储、连接绑定、ACK 和重连语义不变；Relay 不解析计划模式、Diff、Review 或文件内容。

## 0.2.27 — local candidate

- 安装目录精确接受 Runner `0.3.27`，并完整保留 `0.3.26` 与全部原有回滚版本。
- WSS envelope、存储、连接绑定和 ACK 语义不变；本版本只承载 Runner 实时活动刷新、新建任务对账与自动恢复修复的滚动安装身份。

## 0.2.26 — local candidate

- 安装目录精确接受 Runner `0.3.26`，并完整保留 `0.3.25` 与全部原有回滚版本。
- WSS envelope、存储、连接绑定、限流和 ACK 语义不变；Relay 不解析新增的控制阶段语义，也不拥有 Desktop 状态。
...
- 安装目录精确接受 Runner 0.3.24，保留 0.3.23 与全部原有回滚版本。
- 图文命令复用现有 512 KiB WSS 上限及原有身份、摘要、ACK 和重连语义；不增加图片下载 URL、不解析图片正文、不改变 Desktop 状态所有权。

## 0.2.22

- 保持 `0.2.21` WSS 转发、限流、连接绑定与 ACK 语义不变；安装 renderer 新增 Runner `0.3.23` 精确滚动兼容，并完整保留 `0.3.6/0.3.11/0.3.12/0.3.13/0.3.14/0.3.15/0.3.16/0.3.17/0.3.18/0.3.19/0.3.20/0.3.21/0.3.22` 回滚入口。
