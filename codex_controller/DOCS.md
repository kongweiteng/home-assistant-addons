...
当前发布版本：`0.5.48`。正式运行版本仍须以部署后实时核验为准。

## 瞬态 Turn 安全重试

- `0.5.48` 固定 Runner `0.3.35` manifest。Controller 对旧五字段计划模式目录先验证原始摘要，再只删除 `model` / `reasoning_effort`，随后以规范摘要执行完整 Host 和任务快照校验；成功后正常 ACK，其他畸形内容失败关闭。Runner 的实时 Host 只公开三字段目录，内部控制设置仍留在 Mac。页面、Host SSE、原 Thread/Turn、图片、SSE/WSS 增量链路与无 OpenAI API Key 边界不变。Relay `0.2.35` 只增加 `0.3.35` 精确滚动安装兼容，Gateway 保持 `0.4.10`。
...
- 页面只有在 installer manifest URL、options 固定 SHA-256 和公开 WSS Relay URL 全部可用且校验通过时才启用“生成安装命令”。Controller `0.5.48` 镜像内置与 Runner `0.3.35` 四平台制品完全同字节的 manifest，启动期无需访问 GitHub；服务端仍先核对原始字节 SHA-256、版本、完整平台目录和公网 HTTPS URL，再创建 enrollment。任一不匹配都会 fail closed，不会留下无法安装的 Runner 记录。
- manifest v2 固定 Runner `0.3.35`、Codex `0.146.0`、Python `3.11.13` 和 `self_contained=true`，并要求 `linux-amd64`、`linux-aarch64`、`macos-amd64`、`macos-aarch64` 四个平台资产及 installer 自身都有 HTTPS URL、SHA-256 和受限文件大小。Runner 的结构化结果 Schema 要求全部属性都在 `required` 中，`error_code` 成功时为 `null`、失败时为稳定错误码。
...
| `runner_installer_manifest_url` | Runner `0.3.35` 自包含安装制品 manifest v2 的公开 HTTPS URL |
