...
- `0.5.8` 固定 Runner `0.3.5`。临时 `controller_unavailable` 等 Relay 认证错误改为进程内指数退避重连，永久凭据错误仍立即失败；已发送未 ACK 的心跳不会被下一次心跳删除，过期心跳会清理且迟到 ACK 可安全忽略。长任务继续在工作线程执行，WSS 主循环保持 `busy` 心跳。
