...
- `0.5.45` 将内置 manifest 固定到 Runner `0.3.32`。Runner 使用独立 durable `desktop_host` 上行发布 Host、App Bridge、能力和健康，冷启动、任务繁忙或历史读取阻塞时仍可恢复；Controller 以单调序列和摘要仲裁 Host，且连接心跳不会伪装任务数据新鲜。页面实时显示 Bridge 恢复次数、倒计时、最近尝试/成功与稳定错误码，并提供错误中心和 Runner 管理入口；390px 手机视口保持无横向溢出和 44px 触控入口。配套 Relay `0.2.32`，Gateway 保持 `0.4.10`；原 Thread/Turn、图文、SSE/WSS、unknown 不重放和无 OpenAI API Key 边界不变。源码发布不代表 HAOS、Mac Runner 或公网入口已完成升级验收。
...
- `0.5.41` 将内置 manifest 固定到 Runner `0.3.28`，补齐原生 fork/Review、受限 Diff、文件与产物、审批/结构化提问、权限收紧和计划/默认模式；移动与 Web 公用聊天优先的浅色响应式页面，实时数据继续只用 SSE/WSS 增量推送。配套 Relay `0.2.28`、Gateway `0.4.10`，不要求 OpenAI API Key。本节为发布前候选，不代表 HAOS、Mac Runner 或公网入口已升级。
- `0.5.40` 固定 Runner `0.3.27`：活动任务刷新与全局最近任务发现分离，新建 Thread/Turn 使用原生 ID 有界对账，瞬态 read rejection 不再终止 owning app-server；错误中心同时显示 429 重试耗尽与新建/Turn 的稳定脱敏原因。配套 Relay `0.2.27` 只增加精确安装兼容；实时页面继续使用 SSE，Mac 数据面继续使用持久 WSS，不要求 OpenAI API Key。本节为发布前候选，不代表 HAOS、Mac Runner 或公网入口已经升级。
...
- `0.5.36` 按方案 1 将默认入口设为任务，Web 两栏、手机列表/全屏对话；工具、运行设备与设置分开。既有对话和新建任务均可选图或粘贴、预览移除、仅图发送及按需查看历史图片；配套 Runner `0.3.24`/Relay `0.2.23`，继续复用 Mac 登录，不要求 OpenAI API Key。新建回执走 SSE，不重复 POST 轮询。
