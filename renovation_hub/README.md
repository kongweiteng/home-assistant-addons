...
Renovation Hub 是一个独立、确定性、单写入者的 Home Assistant Add-on，用于管理装修记账、询价报价、项目、装修阶段、空间、施工时间线和图片视频档案。
...
- 七个独立应用页面：总览、时间线、资金账目、询价报价、图片视频、装修阶段和设置。
- 询价报价与 Ledger 严格隔离：同一物品可记录多家供应商、规格、联系人、电话、地址、交期、税费、有效期、名片、报价单和商品原图；选择供应商只保存决策，不自动生成账目。
- 图片识别产生的询价或报价默认可标记为“待确认”；页面可查看原图、缩放、前后切换并保留原文件名下载。
...
- 微信装修进度支持跨多条消息连续采集：明确开始后可分开发送图片、视频和说明；先发图片再明确归档也会升级为持久采集会话，只有 Gateway 收到、Hub 存储和事件关联数量完全一致时才完成。
- 页面与 Codex 工具复用同一业务层、幂等键、乐观版本、单 writer 和审计规则。
- 46 个公开业务工具由 Hub 单一 registry 同时生成 MCP manifest 与确定性 dispatch；Controller 可在运行期发现新增工具，不需要维护第二份静态装修目录。九个报价工具支持微信创建、查询、比较、修改、选择和关联媒体；统一变更工具继续支持项目、阶段、空间、时间线和付款的预览确认及批量修改。
- 历史账单可补充项目、阶段和空间归属；批量修改支持按旧分类、日期、项目或账单 ID 筛选，并为每次预览/应用保留审计记录。
- 账单分类独立保存 `category`、`subcategory`、`expense_type`，分类选项由 catalog 管理；“露台”和“凉亭”等业务大类不再依赖标签互相区分。
- 付款计划以项目为边界管理订金、尾款和付款节点，金额从 Ledger 关联流水派生，避免重复合并账单。
- `ledger_export` 返回受控 ZIP 文件制品；内部路径只留在 Hub/Controller，Gateway 发送微信原生文件，失败时才使用短期回退链接。
- 统一搜索可同时检索账目、施工时间线、媒体和询价报价；媒体 list/show 只返回业务元数据与受控内容 URL，不暴露存储文件名、来源摘要或私有路径。
...
- 版本：`0.3.3`，连续采集候选；保留 `0.3.0`～`0.3.2` 的询价报价、跨文件系统媒体落盘和单图查看器修复。
...
python3 -m unittest \
  ...
  tests.test_renovation_dynamic_mcp \
