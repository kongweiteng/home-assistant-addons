...
def create_server(
    ...
    *,
    ...
    class Handler(BaseHTTPRequestHandler):
        server_version = "CodexController/0.3.0"
...
<div id="runnerDisabled" class="card notice"><strong>Runner Center v2 未启用</strong><p class="muted">默认关闭；现有 Codex 对话、MCP、Remote Work v1、微信 Poller、通知和装修业务链路保持原行为。</p></div>
...
q('closeRunnerSecret').onclick=()=>{q('runnerSecretValue').textContent='';q('runnerSecret').classList.add('hidden')};q('runnerStateFilter').onchange=renderRunners;q('runnerPlatformFilter').onchange=renderRunners;q('reloadRunners').onclick=refreshRunners;q('serviceFilter').onchange=renderTools;q('riskFilter').onchange=renderTools;q('reloadTools').onclick=refresh;q('login').onclick=async()=>{try{await call('api/auth/device/start');await refresh()}catch(e){alert(e.message)}};q('cancel').onclick=async()=>{try{await call('api/auth/device/cancel');await refresh()}catch(e){alert(e.message)}};q('retryApiKey').onclick=async()=>{try{await call('api/auth/api-key/retry');await refresh()}catch(e){alert(e.message)}};q('logout').onclick=async()=>{if(confirm('确认退出 HAOS Controller 的独立 Codex 会话？'))try{await call('api/auth/logout');await refresh()}catch(e){alert(e.message)}};refresh();setInterval(refresh,5000);"""
