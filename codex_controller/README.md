...
- `0.5.16` 允许同一 Desktop `thread_revision` 仅刷新由 Owner/IPC 可用性派生的 `status` 与 `control_state`，解除合法恢复快照被误判为 `desktop_revision_conflict` 的队头阻塞。其他业务字段、Thread/Runner/host/project 绑定、摘要、隐私与未来 revision 仍严格失败关闭。
