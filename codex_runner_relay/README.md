...
`0.2.37` 将安装 renderer 的精确滚动兼容扩展到 Runner `0.3.37`，并完整保留 `0.3.36` 与全部原有回滚版本。Relay 的 WSS、ACK、连接身份与失败关闭边界不变；任务目录分页和 App-owned 管理后台增强均不在 Relay 解析或所有权范围内。源码发布不代表 HAOS 或 Mac Runner 已完成升级验收。
...
安装 renderer 精确接受 Runner `0.3.37`，并保留已发布 `0.3.6`、`0.3.11` 至 `0.3.36` 安装目录中已登记的全部滚动兼容版本；除此之外的版本仍 fail closed。Relay 到 Controller 的 IPv4、Registry/lease 所有权、ticket/enrollment 不落盘、单 Runner 连接绑定和其他错误失败关闭边界保持不变。
