...
class RunnerInstallerCatalogTests(unittest.TestCase):
    ...
    def test_valid_pinned_manifest_builds_single_line_linux_and_macos_commands(self) -> None:
        ...
        self.assertEqual(linux["runner_version"], "0.3.30")
    ...
    def test_pinned_manifest_body_avoids_runtime_network_dependency(self) -> None:
        ...
        self.assertEqual(status["runner_version"], "0.3.30")
    ...
    def test_packaged_runner_0330_manifest_matches_frozen_candidate_digest(self) -> None:
        package_root = Path(codex_controller.__file__).parent
        body = (package_root / "runner_manifest_v0330.json").read_bytes()
        self.assertEqual(
            hashlib.sha256(body).hexdigest(),
            "e504bcca26dfab70ea9cb735c1aafb96593677e791ba4ac353f4e0cd2d6178b7",
        )
        self.assertIn(
            'with_name("runner_manifest_v0330.json")',
            (package_root / "main.py").read_text(encoding="utf-8"),
        )
        catalog = RunnerInstallerCatalog(
            "https://github.com/example/project/releases/download/codex-runner-v0.3.30/manifest.json",
            hashlib.sha256(body).hexdigest(),
            "wss://runner.example.com/v1/connect",
            pinned_manifest_body=body,
            opener=ForbiddenOpener(),
