...
class RunnerInstallerCatalogTests(unittest.TestCase):
    ...
    def test_valid_pinned_manifest_builds_single_line_linux_and_macos_commands(self) -> None:
        ...
        self.assertEqual(linux["runner_version"], "0.3.17")
    ...
    def test_pinned_manifest_body_avoids_runtime_network_dependency(self) -> None:
        ...
        self.assertEqual(status["runner_version"], "0.3.17")
    ...
    def test_packaged_runner_0317_manifest_matches_frozen_candidate_digest(self) -> None:
        body = Path(codex_controller.__file__).with_name("runner_manifest_v0317.json").read_bytes()
        self.assertEqual(
            hashlib.sha256(body).hexdigest(),
            "68e632fb7598165aef53e53beae1d21d25ec05c4cb5e753da5484076d049d03c",
        )
        catalog = RunnerInstallerCatalog(
            "https://github.com/example/project/releases/download/codex-runner-v0.3.17/manifest.json",
            hashlib.sha256(body).hexdigest(),
            "wss://runner.example.com/v1/connect",
            pinned_manifest_body=body,
            opener=ForbiddenOpener(),
