...
`0.2.33` 将安装 renderer 的精确滚动兼容扩展到 Runner `0.3.33`，并完整保留 `0.3.31` 与全部原有回滚版本。Relay 允许并原样转发独立 `desktop_host` 上行，只对 Controller 明确返回的 stale Host 序列安全 ACK；它不解析 Host、Bridge 或任务内容，也不改变连接身份和权限边界。源码发布不代表 HAOS Relay 或 Mac Runner 已完成升级验收。
