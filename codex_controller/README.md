...
- `0.5.28` 修复旧 outbox 的精确同 revision 阻塞：只有双方 `control_revision` 都显式为 null、唯一变化是不可写 `control_state` 时才刷新；所有可写升级与业务/历史漂移继续失败关闭。内置 manifest 固定 Runner `0.3.20`，配套 Relay `0.2.19`；新版 App continue 使用 v2 + `turnStart`，旧三项按其审计 tuple 继续使用 v1 + `turnStartParams`，未知版本保持只读。
