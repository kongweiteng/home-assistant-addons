...
class DesktopApiTests(unittest.TestCase):
    ...
    def test_desktop_ingress_page_assets_and_privacy_contract(self) -> None:
        ...
        self.assertIn("color-scheme:only light", body)
        ...
        self.assertIn(".mobile-nav{grid-template-columns:repeat(5,1fr)}", body)
        self.assertIn("@media(max-width:759px)", body)
        self.assertIn(".queue-panel:not(.open) .queue-list{display:none}", body)
        ...
        self.assertIn('<a href="../?view=overview">', body)
        ...
        self.assertIn('id="connectionBridgeRecovery"', body)
        self.assertIn('id="connectionBridgeHealth"', body)
        self.assertNotIn('id="connectionBridgeAttempt"', body)
        self.assertNotIn('id="connectionBridgeSuccess"', body)
        ...
        self.assertIn("继续检查", script)
        self.assertNotIn("for (let attempt = 0; attempt < 45; attempt += 1)", script)
        self.assertIn("document.create_commands", script)
        self.assertIn("handleCreateResult", script)
        self.assertIn("原生快速调整保持同一 Turn，不允许切换模型", script)
        self.assertIn("new EventSource", script)
        self.assertIn("document.scope?.type !== 'host'", script)
        self.assertIn("document.scope?.type !== 'thread'", script)
        ...
        self.assertIn("visibilitychange", script)
        self.assertNotIn("wait_seconds=20", script)
        self.assertNotIn("setInterval(() => void refreshOverview", script)
        self.assertIn("thread_queue_v1", script)
        ...
        self.assertIn("后台连接探测中", script)
        self.assertIn("被动重连 · 不会切换桌面窗口", script)
        self.assertNotIn("recovery_attempt_count", script)
        self.assertNotIn("last_activation_success", script)
        self.assertIn("Controller 已接收", script)
        self.assertIn("Relay 已送达", script)
        self.assertIn("Runner 已接收", script)
        self.assertIn("Mac 已确认", script)
        self.assertIn("const head = document.createElement('button')", script)
        self.assertIn("head.setAttribute('aria-expanded'", script)
        ...
        self.assertIn("querySelector('main.shell').inert", script)
        self.assertIn("restoreFocus", script)
