...
`0.4.10` 修复旧失败记录的来源识别：只要已有 Controller 作业短引用，即使记录来自结构化状态字段加入前，也按 Controller 故障展示其稳定错误码，不会误报为微信投递失败。管理状态 SSE、断线重连、单 Poller 与唯一 notification consumer 保持不变。
...
从 `0.4.10` 回退到 `0.4.9` 不需要数据库降级，但旧失败记录可能再次被误标为微信投递失败。继续回退到 `0.4.8` 时页面会恢复 5 秒状态轮询，但失败目录、消息、身份、Poller、artifact、Runner Manager 和通知台账保持。继续回退到 `0.4.7` 时旧版本会忽略新增的 Controller 状态与错误码列，并不再提供失败目录。
