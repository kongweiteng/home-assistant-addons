...
function transactionCategory(item: Transaction): string {
  ...
  return item.main_category.trim() || "未分类";
}

function transactionKind(item: Transaction): "付款" | "订金" | "退款" {
  if (item.type === "refund") return "退款";
  return item.is_deposit ? "订金" : "付款";
}
...
export function App() {
  ...
  const loadProject = useCallback(async (targetProjectId: string) => {
    ...
    setError("");
    try {
      const [dashboard, stages, areas, timeline, ledger, media, catalog, paymentPlans, quotes] = await Promise.all([
...
function UndoDialog({ item, saving, onClose, onSubmit }: { item: Transaction; saving: boolean; onClose: () => void; onSubmit: (item: Transaction, reason: string) => void }) {
  ...
  return <Dialog title="撤销账目" subtitle="撤销会保留原流水和审计记录，不会物理删除。" onClose={onClose}><form onSubmit={(event) => { event.preventDefault(); onSubmit(item, reason); }}><div className="warning-box"><IconTrash size={22} /><span>即将撤销 {transactionKind(item)} · {transactionCategory(item)} · {currency(item.amount_cents)}</span></div><Field label="撤销原因" wide><textarea required rows={3} value={reason} onChange={(event) => setReason(event.target.value)} /></Field><DialogActions saving={saving} onClose={onClose} submitLabel="确认撤销" /></form></Dialog>;
}
