...
- 微信装修进度支持跨消息连续采集：明确开始后可分开发送图片、视频和说明；只有 Gateway received、Hub registered/stored 与事件 linked 完全一致且没有失败/待处理项时才完成。
- 页面与 Codex 工具复用同一业务层、幂等键、乐观版本、单 writer 和审计规则。
- 40 个公开业务工具由 Hub 单一 registry 同时生成 MCP manifest 与确定性 dispatch；Controller 可在运行期发现新增工具，不需要维护第二份静态装修目录。九个报价工具支持微信创建、查询、比较、修改、选择和关联媒体；统一变更工具继续支持项目、阶段、空间、时间线和付款的预览确认及批量修改。
...
- 版本：`0.3.4`，实验候选。完整保留 `0.3.2` 报价与媒体基线，本版新增确定性连续采集；未引入分类目录、付款计划、账本导出或无关前端。
