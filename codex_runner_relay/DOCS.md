...
`0.2.26` 的安装目录新增精确 Runner `0.3.26`，并保留 `0.3.25` 与全部原有回滚版本。WSS envelope、存储、连接绑定及既有 ACK 行为不变：Controller 明确拒绝为 `desktop_revision_conflict` 的旧 `desktop_snapshot` 仍按精确组合返回传输 ACK，使后续心跳和实时事件继续流动；其他冲突继续失败关闭。本版本当前为发布前候选。
...
- 返回脚本下载固定 installer，核对字节大小和 SHA-256，再下载并核对固定平台 bundle。Relay `0.2.26` 的滚动目录精确接受 Python `3.11.13`、Codex `0.146.0` 与 Runner `0.3.26`，同时保留已冻结的 `0.3.6/0.3.11/0.3.12/0.3.13/0.3.14/0.3.15/0.3.16/0.3.17/0.3.18/0.3.19/0.3.20/0.3.21/0.3.22/0.3.23/0.3.24/0.3.25` bootstrap；其他 Runner 版本仍拒绝。
