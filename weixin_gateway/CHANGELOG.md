...
- 旧版本已绑定 Controller 作业但尚无结构化状态字段的失败记录，继续按 Controller 故障展示原稳定错误码，不再误报为微信投递失败。
- 保留 `0.4.9` 的管理状态 SSE、断线重连、单 Poller 和唯一 notification consumer；数据库结构、身份与公网边界不变。

## 0.4.9

- 管理页面状态改为 SSE 长连接：首帧立即返回脱敏有界快照，运行态、队列、身份、二维码和管理操作变化通过 Condition 主动唤醒，空闲时仅发送 12 秒心跳，不再每 5 秒轮询状态、用户和会话。
- 浏览器支持 Last-Event-ID 修订恢复、1～30 秒指数退避、网络恢复和重新回到前台时主动重连；页面明确显示实时链路状态。
- 用户与会话详情只在详情修订变化时各读取一次；补充真实 JavaScript 执行与 390px 手机无横向溢出验收。

## 0.4.8

- 微信任务失败回复增加稳定错误码、中文原因和建议动作，不再只提示前往 Controller；Controller 终态与微信回传失败分别保存，避免错误原因相互覆盖。
- 新增最近 20 条脱敏失败目录，只返回 HMAC 短标识、上海时间、组件、严重度和可重试状态；不返回微信用户、消息正文、凭据或原始异常。
- 新增生产格式 iLink 入站的内存发送器模拟，覆盖接收、提交、Controller 失败、微信回复和失败目录；模拟通过不等于真实手机送达。
