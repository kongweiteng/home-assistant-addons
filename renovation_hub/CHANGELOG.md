...
- 新增持久 `progress_capture_sessions/items/notes` 草稿，以同一事件连续接收跨消息图片、视频和说明，支持暂停、恢复、取消、序号说明、重启恢复与 256 项上限。
- 媒体先预登记再流式写入；图片/视频原件内容寻址保存，处理失败保留原件并阻断完成，可在不替换原件的情况下重试处理。
- 完成事务严格核对 Gateway received、Hub registered/stored 与事件 linked；pending、failed、缺失关联或附件 ACK 未确认时均不会返回完成。
- 保留 `0.3.2` 的报价、Ledger、writer lease 与跨文件系统原子落盘能力；未带入旧候选中的分类目录、付款计划、Portable 下载或无关前端。

## 0.3.2

- 修复只有一张报价图片时，原图画布因左右切换按钮不渲染而落入 64px 导航列、导致图片几乎不可见的问题。
- 原图画布现在固定在查看器中间列；多图的上一张/下一张、缩放和下载行为保持不变。

## 0.3.1

- 修复 HAOS 中 `/data` 上传暂存目录与 `/media` 原图目录位于不同文件系统时，图片完成处理因 `EXDEV` 失败的问题。
- 跨文件系统时先在目标目录写入私有临时文件，复核大小与 SHA-256 后原子落位；失败保留原暂存文件以便安全重试。

## 0.3.0

- 新增与 Ledger 严格隔离的询价报价子系统：同一物品可保存多家供应商、规格、联系人、电话、地址、品牌型号、费用、交期、有效期、付款条件、质保和识别置信度；选择报价不会自动生成账目。
- Hub schema additive 升级到 2，新增询价、供应商报价和报价媒体关联表，并保留幂等、乐观版本、审计、上海时区过期判断和媒体去重；修改费用会重新计算总价与标准化单价，过期报价不可选择。
- 新增九个 `renovation_quote_*` 动态业务工具、报价页面 API 和统一搜索结果；微信可创建、查询、比较、修改、选择报价，并把报价单、名片、地址图或商品规格图归档关联。
- Ingress 新增第七个“询价报价”页面，支持桌面双栏和手机布局、多供应商比较、询价与报价编辑、图片上传，以及原图缩放、切换和下载。
- 新增后端、Controller、页面 API、前端与视觉交互回归；报价选择与 Ledger 计数隔离由测试固定。
- 正式运行记录已经使用 `0.2.9` 身份，而公开 `main` 基线仍为 `0.2.8`；本次以 `0.3.0` 发布，避免不同源码复用同一 Add-on 版本号。
