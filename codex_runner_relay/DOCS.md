...
- 返回脚本下载固定 installer，核对字节大小和 SHA-256，再下载并核对固定平台 bundle。bundle 内置 Python `3.11.13`、Runner `0.3.5` 和 Codex `0.146.0`。
...
先撤销或等待短期安装链接过期，关闭 Controller 的新 enrollment 与 Relay 配置，确认没有活动 lease 或 `recovery_required`，再停止 Relay。回退到 `0.2.6` 会重新只接受 Runner `0.3.4`，因此必须同时回退 Controller manifest，不能继续生成 `0.3.5` 安装链接。停止 Relay 不删除 Controller Registry、Runner credential 摘要、服务器 Agent、worktree、分支或 Session。
