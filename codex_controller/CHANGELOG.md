...
- Runner 将首轮目录与活跃任务详情解耦：同项目、同归档域的重复投影仅在更新时间严格可排序时保留最新项，历史任务不再逐项加载队列和完整 Owner 历史，`data_synced_at` 与安全写入状态可在大型任务库启动后快速恢复。
- Bridge 慢响应使用连续失败阈值、最近成功保鲜及后台唤醒安静期，避免桌面 Codex 窗口反复跳转；归档只读取目标任务。Relay `0.2.36` 仅增加 Runner `0.3.36` 精确安装兼容，Gateway 保持 `0.4.10`。
...
- 保留 `0.5.42` 的第一方 `read_thread` 校验、浅色移动/Web 工作台、SSE/WSS、图文、新建/继续、审批/提问、Diff/资源、模式与权限边界；Gateway 保持 `0.4.10`，Relay `0.2.30` 只增加 Runner `0.3.30` 精确滚动兼容。
...
- 首次历史请求改为处理 SSE `ready` 帧后提交，消除只等 HTTP 连接打开的首连竞态。
- Relay 发布结果不确定，或断线重连时请求结果已超出 SSE 事件窗口时，页面解锁操作并显示手动重试；不自动重放未知的历史请求。
- 内置 manifest 与安装身份提升到 Runner `0.3.25`，正式声明原生 app-server 分页/搜索与脱敏 cursor 能力；配套 Relay `0.2.25` 只增加该 Runner 的精确滚动安装兼容，既有 WSS 与 ACK 契约不变。

## 0.5.37

- 新增 Controller 错误中心：总览显示最近脱敏问题，独立状态页按组件状态与 Mac 任务上下文分组并增量分页；错误条目只包含稳定错误码、短引用、上海时间与是否可重试，不显示微信身份、消息正文、URL、凭据或原始异常。
- 错误状态通过独立 SSE `error_revision` 实时推送；页面断线重连后只重取最新页，不以长轮询或一次性全量错误列表替代。
- 微信失败仅由 Controller 使用既有 Gateway 内部地址和 Bearer 凭据读取最小受保护失败目录；浏览器不会访问或暴露 Gateway 管理 API。
- 较早对话和任务内公开文本搜索改用 Mac App 原生游标；每页最多 20 Turn，结果按 request ID 经现有 Thread SSE 合并，初始历史请求只在事件流建立后提交，避免快速回执被首连游标跳过。
- Runner `0.3.24` 与 Relay `0.2.23` 兼容身份不变。

## 0.5.36

- 方案 1 任务优先布局：Web 列表/对话双栏，项目按需选择；手机全屏对话与任务、工具、新建、状态、设置五入口，MCP 改为可搜索、分组折叠的增量目录。
- 既有对话支持选择/粘贴 PNG、JPEG、WebP，最多四图、每图 64 KiB；发送前预览/移除、仅图片消息、24 小时过期重传与按需读取历史图片。HA Ingress 身份边界和图片读写 CSRF 保护不变，不接受远程图片 URL 或本地路径。
- image_input_v1 通过既有 WSS 发送到 Mac，支持 continue/create/safe steer；queue/native 不接受图片，旧 Runner 明确禁用入口。SSE、公开历史元数据和命令日志不包含图片正文，未知结果保留同一请求编号。
- 新建任务取消重复 POST 查询，改为一次提交与有界 host SSE 回执；重连 ready 恢复最近 20 条状态，不自动重放控制。修复快速切任务、切筛选时旧响应覆盖和回执误清其他草稿。
- 图文配套固定 Runner 0.3.24 manifest；Relay 0.2.23 保留旧版本滚动兼容。源码/本地验收与正式 HAOS/实体手机验收分别记录。

## 0.5.35

- 将 Controller 总览和 Desktop 工作台统一为 SSE 持久连接：host、Thread、总览状态只推送增量变化，12 秒心跳、指数退避、前台/网络恢复与游标续传共同保证页面打开期间自动恢复；不再使用页面长轮询或固定 5 秒刷新。
- 修复结构化 `scope` 被前端误判、`changed=true/events=[]` 变化丢失、事件裁剪后旧游标静默漏数和总览连接徽标忽略任务数据年龄的问题。裁剪或连续失败时执行有界 baseline 对账，项目/状态筛选重新请求服务端分页。
- 完成浅色移动优先信息架构：宽屏为项目、任务、对话三栏，手机/平板为列表到全屏对话的单栏；底部任务/项目/新建/状态/设置导航、新建任务 sheet、常驻 composer、公开思考摘要、模型/推理强度、队列、停止和归档保持单手可用。
- 首屏只读取最近 40 个任务，后续滚动分页；实时帧不携带全量历史。仍只展示 Runner 净化后的公开 reasoning summary，不传输隐藏思维过程、原始 Thread/Turn 标识、路径或凭据；复用 Mac Codex App 登录，不要求 OpenAI API Key。
- 配套 Runner `0.3.23` 使用四路持久 app-server 会话池和活动任务有界并发同步，单个慢任务不再阻塞其他任务；Relay `0.2.22` 增加 `0.3.23` 精确滚动安装兼容，数据面与 ACK 语义不变。
...
- 新增按 host 的 Desktop 总览事件长轮询；任务新增、状态变化和收据到达可立即唤醒列表刷新。页面分别展示 Runner/WSS 连接观测时间与 Desktop 数据同步时间，并保留前台恢复、网络恢复和有界兜底同步。
- 消息发送新增 Controller 已接收、Relay 已送达、Runner 已接收、Mac 已确认四阶段状态；任务详情持续展示 Runner 已净化的公开思考摘要，按模型目录提供推理强度选择。隐藏 reasoning、原始 Thread/Turn ID、绝对路径和凭据仍不得进入 HAOS。
- 活动任务新增纯文本排队消息的添加、编辑、排序和删除，并使用 request ID、revision/CAS 与收据保持幂等和冲突失败关闭；手机端修正窄屏布局、底部输入、安全区和单栏返回交互。新建任务、续写和快速调整继续复用 Mac Codex App 登录，不新增 OpenAI API Key 依赖。

## 0.5.32

- 将 `/desktop` 从技术事件面板重构为聊天优先的实时工作台：用户消息与 Codex 回复进入同一连续对话，活动 `assistant.delta` 直接显示为正在回复，计划、命令、文件变化和公开摘要默认折叠为“运行详情”。
- 移除主界面手动刷新入口；事件长轮询每次返回都会重读任务详情，总览每 8 秒自动同步，并在页面回到前台或网络恢复时立即同步和重启事件流。用户向上阅读时不抢滚动，改用“查看新回复”提示。
- 手机端保持任务列表到对话详情的单栏切换，输入框固定于底部，支持 Enter 发送、Shift+Enter 换行与自动增高；模型、快速调整、中断和归档收进高级/更多区域。request ID、revision/CAS、收据、草稿与 unknown 不自动重放语义保持不变，也不新增 OpenAI API Key 依赖。
