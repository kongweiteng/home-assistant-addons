...
HA Manager Executor 是 Home Assistant Supervisor `manager` 权限域的独立执行器骨架。`0.1.0` 只提供经过内部 bearer 认证的 `restart_addon` 只读 shadow 预检：读取精确白名单 Add-on 的状态、生成绑定摘要并验证基线，不调用 restart/start/stop/configure/install/update 等任何写端点。
