...
class RelayProtocolUnitTests(unittest.TestCase):
    def test_addon_version_and_controller_hostname_contract_are_exact(self) -> None:
        ...
        self.assertIn('version: "0.2.38"', config)
        self.assertEqual(
            SUPPORTED_RUNNER_VERSIONS,
            frozenset(
                {"0.3.6", "0.3.11", "0.3.12", "0.3.13", "0.3.14", "0.3.15", "0.3.16", "0.3.17", "0.3.18", "0.3.19", "0.3.20", "0.3.21", "0.3.22", "0.3.23", "0.3.24", "0.3.25", "0.3.26", "0.3.27", "0.3.28", "0.3.29", "0.3.30", "0.3.31", "0.3.32", "0.3.33", "0.3.34", "0.3.35", "0.3.36", "0.3.37", "0.3.38"}
            ),
        )
...
class FakeController:
    ...
    async def install_bootstrap(self, ticket: str) -> dict:
        ...
        return {
            ...
            "labels": ["local", "macos"],
            "policy_revision": 2,
            "asset_url": "https://downloads.example.com/codex-runner-0.3.12-macos-aarch64.tar.gz",
            "asset_sha256": "a" * 64,
            "asset_size": 123456,
...
class RelayIntegrationTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_install_link_renders_no_store_digest_pinned_shell_without_logging_ticket(self) -> None:
        ...
        self.assertIn("codex-runner-0.3.12-macos-aarch64.tar.gz", body)
