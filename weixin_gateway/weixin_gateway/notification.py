"""Model-free MQTT v1 notifications delivered through the bound Weixin owner."""
...
LOGGER = logging.getLogger("weixin_gateway_notification")
...
PROTOCOL_VERSION = 1
MQTT_CLIENT_ID = "weixin-gateway-notification-v1"
...
class NotificationConfig:
    ...
    @classmethod
    def from_env(cls, env: Mapping[str, str] = os.environ) -> "NotificationConfig":
        host = env.get("WEIXIN_NOTIFICATION_MQTT_HOST", "").strip()
        username = env.get("WEIXIN_NOTIFICATION_MQTT_USERNAME", "").strip()
        password = env.get("WEIXIN_NOTIFICATION_MQTT_PASSWORD", "")
        data_dir_raw = env.get("WEIXIN_DATA_DIR", "").strip()
        if not host:
            raise ValueError("notification MQTT host is required")
        if not username or not password:
            raise ValueError("notification MQTT username and password are required")
        if not data_dir_raw:
            raise ValueError("WEIXIN_DATA_DIR is required")

        try:
            port = int(env.get("WEIXIN_NOTIFICATION_MQTT_PORT", "1883"))
        except ValueError as exc:
            raise ValueError("notification MQTT port must be an integer") from exc
        if not 1 <= port <= 65535:
            raise ValueError("notification MQTT port must be from 1 to 65535")

        tls_value = env.get("WEIXIN_NOTIFICATION_MQTT_TLS", "false").strip().lower()
        if tls_value not in {"true", "false"}:
            raise ValueError("notification MQTT TLS must be true or false")

        audiences = frozenset(
            value.strip()
            for value in env.get("WEIXIN_NOTIFICATION_ALLOWED_AUDIENCES", "owner").split(",")
            if value.strip()
        )
        if audiences != frozenset({"owner"}):
            raise ValueError("notification audience v1 must be exactly owner")
        ...
        return cls(
            ...
            ledger_path=Path(data_dir_raw) / "notification-ledger.sqlite3",
...
def parse_request(
    ...
    *,
    ...
    if set(payload) != _REQUEST_FIELDS:
        raise RequestValidationError(
            "invalid_payload", "payload fields must match notification v1"
        )
    ...
    if payload.get("version") != PROTOCOL_VERSION:
        raise RequestValidationError(
            "unsupported_version", "version must be 1", message_id=message_id
        )
    ...
    message = _require_string(payload, "message", max_length=4000, message_id=message_id)
...
def format_weixin_text(request: NotificationRequest) -> str:
    """Return the exact model-free Weixin text."""
    return f"【{_LEVEL_LABELS[request.level]}】{request.title}\n{request.message}"


class NotificationLedger:
    """Persist routing state without retaining notification bodies or identities."""
    ...
    def __init__(self, path: Path) -> None:
        ...
        with self._db:
            ...
            self._db.execute(
                "CREATE INDEX IF NOT EXISTS idx_notification_dedupe ON messages(dedupe_key, received_at)"
            )
            self._db.execute(
                "CREATE INDEX IF NOT EXISTS idx_notification_source ON messages(source, received_at)"
            )
        path.chmod(0o600)
...
def discovery_messages(addon_version: str) -> dict[str, dict[str, Any]]:
    device = {
        "identifiers": ["weixin_gateway_notification_v1"],
        "name": "Weixin Gateway Notification",
        "manufacturer": "Weixin Gateway Add-on",
        "model": "MQTT Weixin notification adapter",
        "sw_version": addon_version,
    }
    origin = {
        "name": "Weixin Gateway Add-on",
        "sw_version": addon_version,
    ...
    return {
        f"{DISCOVERY_PREFIX}/binary_sensor/weixin_gateway_notification_online/config": {
            "name": "Online",
            "unique_id": "weixin_gateway_notification_online",
            "default_entity_id": "binary_sensor.weixin_gateway_notification_online",
        ...
        f"{DISCOVERY_PREFIX}/sensor/weixin_gateway_notification_last_result/config": {
            "name": "Last result",
            "unique_id": "weixin_gateway_notification_last_result",
            "default_entity_id": "sensor.weixin_gateway_notification_last_result",
            "entity_category": "diagnostic",
...
class NotificationProcessor:
    """Validate, deduplicate, deliver, and report one request at a time."""
    ...
    def __init__(
        ...
        *,
        ...
        sender: Callable[[NotificationRequest, str], tuple[bool, Optional[str], bool]],
    ...
    def process(self, raw_payload: bytes) -> bool:
        """Return true only after the final result is durably published."""
        try:
        ...
        try:
            ...
            message_id = (
                exc.message_id
                if _ID_RE.match(exc.message_id)
                else f"invalid-{uuid.uuid4().hex}"
            )
        ...
        if existing:
            ...
            if (
                source_count >= DEFAULT_SOURCE_RATE_LIMIT
                or global_count >= DEFAULT_GLOBAL_RATE_LIMIT
            ):
        ...
        for delay in (0, *self.retry_delays):
            if delay:
                ...
                self.ledger.update(
                    ...
                    status="retrying",
                    attempts=attempts,
                    error_code="notification_retry_scheduled",
                )
                self._publish(
                    request.message_id,
                    "retrying",
            ...
            ok, error_code, recoverable = self.sender(
                request, format_weixin_text(request)
            )
            if ok:
                return self._finish(
...
class GatewayNotificationRuntime:
    """MQTT v5 persistent consumer with one model-free delivery worker."""
    ...
    def __init__(
        ...
        *,
        ...
        self.ledger = NotificationLedger(config.ledger_path)
        self._closed = False
        ...
        self.processor = NotificationProcessor(
            ...
            sender=self._send_via_gateway,
            sleeper=self._interruptible_sleep,
        )
        self.worker = threading.Thread(
            target=self._worker_loop,
            name="weixin-notification-worker",
            daemon=True,
        )
        self.network = threading.Thread(
            target=self._network_loop,
            name="weixin-notification-mqtt",
            daemon=True,
        )
    ...
    def _publish_discovery(self) -> None:
        for topic, payload in discovery_messages(self.config.addon_version).items():
            self._publish_json(topic, payload, retain=True, wait_for_publish=False)

    def _send_via_gateway(
        self, request: NotificationRequest, text: str
    ) -> tuple[bool, Optional[str], bool]:
        if self.stop_event.is_set():
            return False, "delivery_state_unknown", False
        future = asyncio.run_coroutine_threadsafe(
            self.service.send_notification(request.message_id, text), self.loop
        )
        try:
            ...
            future.cancel()
            # The iLink request may already have reached Weixin even though the
            # Controller-side wait expired. Retrying could therefore duplicate
            # a user-visible notification.
            return False, "delivery_state_unknown", False
        except StoreError as exc:
            return False, exc.code, False
        except ProtocolError as exc:
            if exc.code == "send_rate_limited":
    ...
    def _on_disconnect(
        ...
        if not self.stop_event.is_set():
            LOGGER.warning("Notification MQTT disconnected: %s", reason_code)
    ...
    def _on_message(self, client: Any, userdata: Any, message: Any) -> None:
        ...
        try:
            ...
            LOGGER.error("Notification queue is full; request left unacknowledged")
    ...
    def _worker_loop(self) -> None:
        while not self.stop_event.is_set():
            try:
                ...
                continue
            try:
                if self.processor.process(message.payload) and message.qos > 0:
    ...
    def _network_loop(self) -> None:
        try:
            ...
            if not self.stop_event.is_set():
                LOGGER.exception("Notification MQTT loop stopped unexpectedly")
