...
- `0.5.20` 完整保留正式 `0.5.19` 的业务源码、53 个工具、瞬态 Turn 重试、M8 owner-only 备车和 Desktop availability refresh，只把 Controller 内置 pinned manifest 与 Runner 版本常量精确同步到已发布的 Runner `0.3.14`。仅修改 options URL/SHA 不能替换内置 manifest；版本、原始字节 SHA-256 或四平台目录任一不匹配仍 fail closed。
