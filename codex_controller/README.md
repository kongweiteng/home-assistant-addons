...
- `0.5.11` 扩展明确媒体归档意图到询价、报价单、价格单、供应商名片和商品规格；普通附件仍只用于本轮识别，只有用户明确要求保存或关联报价资料时才开放 Hub 媒体链路。Hub `0.3.0` 的九个 `renovation_quote_*` 动态工具由现有 manifest 自动进入 owner 微信会话，可创建、查询、比较、修改和选择报价。
